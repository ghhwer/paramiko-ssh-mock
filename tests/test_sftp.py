...
def test_example_function_sftp_write():
    ...
    with patch('paramiko.SSHClient', new=SSHClientMock):
        ...
        mock_file = ParamikoMockEnviron().get_mock_file_for_host(
            'some_host_4', 22, '/tmp/afileToWrite.txt'
        )
...
def test_example_function_sftp_read():
    ...
    ParamikoMockEnviron().add_mock_file_for_host(
        'some_host_4', 22, '/tmp/afileToRead.txt', mock_file
    )
    # patch the paramiko.SSHClient with the mock
    with patch('paramiko.SSHClient', new=SSHClientMock):
...
def test_example_function_sftp_put():
    ...
    with patch('paramiko.SSHClient', new=SSHClientMock):
        ...
        # Get the file content from the mock filesystem
        mock_file = ParamikoMockEnviron().get_mock_file_for_host(
            'some_host_4', 22, '/remote/path/to/file_a.txt'
        )
        # Check if the content is the same as the one written
...
def test_example_function_sftp_get():
    ...
    # Add the mocked file to the host
    ParamikoMockEnviron().add_mock_file_for_host(
        'some_host_4', 22, '/remote/path/to/file_b.txt', mock_file
    )
    # patch the paramiko.SSHClient with the mock
    with patch('paramiko.SSHClient', new=SSHClientMock):
        example_function_sftp_get()
        # Get the file content from the mock filesystem
        mock_local_file = ParamikoMockEnviron().get_local_file(
...
def test_example_function_sftp_list():
    ...
    for file in files:
        ParamikoMockEnviron().add_mock_file_for_host(
            'some_host_4', 22, file, LocalFileMock()
        )
...
def test_sftp_put_callback():
    ...
    sftp_client.put(
        '/local/testfile.txt', '/remote/testfile.txt', callback=callback
    )
...
def test_sftp_get_callback():
    ...
    sftp_client.get(
        '/remote/testfile.txt', '/local/testfile.txt', callback=callback
    )
